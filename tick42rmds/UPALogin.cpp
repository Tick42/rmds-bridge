...
RsslRet UPALogin::processLoginResponse(RsslChannel* chnl, RsslMsg* msg, RsslDecodeIterator* dIter)
{
    ...
    case RSSL_MC_REFRESH:
        ...
        if ((ret = DecodeLoginResponse(&loginResponseInfo_, msg, dIter)) != RSSL_RET_SUCCESS)
        {
            ...
            NotifyListeners(&loginResponseInfo_, false, buff.data);
        ...
        if (loginResponseInfo_.isSolicited && pState->streamState == RSSL_STREAM_OPEN && pState->dataState == RSSL_DATA_OK)
        {
            ...
            NotifyListeners(&loginResponseInfo_, true, ToString(pState).c_str());
        }
        else /* handle error cases */
        {
            if (pState->streamState == RSSL_STREAM_CLOSED_RECOVER)
            {
                ...
                NotifyListeners(&loginResponseInfo_, false, ToString(pState).c_str());
            ...
            else if (pState->streamState == RSSL_STREAM_CLOSED)
            {
                ...
                t42log_error("\n%s\n",errMsg);
                NotifyListeners(&loginResponseInfo_, false, errMsg);
    ...
    case RSSL_MC_STATUS:
        ...
        if (msg->statusMsg.flags & RSSL_STMF_HAS_STATE)
        {
            ...
            if (pState->streamState == RSSL_STREAM_CLOSED_RECOVER)
            {
                const char * errMsg = "Login Failed - Login stream is closed recover";
                t42log_error("\n%s\n",errMsg);
                NotifyListeners(&loginResponseInfo_, false, errMsg);
            ...
            {
                const char * errMsg = "Login Failed - Login stream is closed";
                t42log_error("\n%s\n",errMsg);
                NotifyListeners(&loginResponseInfo_, false, errMsg);
            ...
            {
                const char * errMsg = "Login Failed - Login stream is suspect";
                t42log_error("\n%s\n",errMsg);
                NotifyListeners(&loginResponseInfo_, false, errMsg);
...
void UPALogin::NotifyListeners( RsslLoginResponseInfo *pResponseInfo, bool loginSucceeded, const char* extraInfo)
{
    ...
    {
        (*it)->LoginResponse(pResponseInfo, loginSucceeded, extraInfo);
        it++;
    }
