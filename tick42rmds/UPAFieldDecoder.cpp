	...
	case RSSL_DT_REAL:
		{
			...
			if ((ret = rsslDecodeReal(dIter, &realVal)) == RSSL_RET_SUCCESS)
			{
				...
				else
				{
					...
					AddRsslStringToMsg(msg, mamaField, string(realBuff.data), fEntry->fieldId);
				}
			...
			else
			{
				...
				else
				{
					...
					AddRsslStringToMsg(msg, mamaField, string(""), fEntry->fieldId);
				}
			}
			break;
		}
	...
	case RSSL_DT_ENUM:
		{
			...
			if ((ret = rsslDecodeEnum(dIter, &enumVal)) == RSSL_RET_SUCCESS)
			{
				...
				if (pEnumType != 0)
				{
					...
					AddRsslStringToMsg(msg, mamaField, strEnumVal, fEntry->fieldId);
				}
				else
				{
					// enum lookup failed, just put the number value int he message
					AddRsslUintToMsg(msg, mamaField, enumVal, fEntry->fieldId);
				}
			}
			else if (ret != RSSL_RET_BLANK_DATA)
			{
...
RsslRet UPAFieldDecoder::DecodeBookFieldEntry(RsslFieldEntry* fEntry, RsslDecodeIterator *dIter, UPABookEntry_ptr_t entry)
{
	RsslRet ret = 0;
...
mama_status UPAFieldDecoder::AddRsslStringToMsg( mamaMsg msg, MamaField_t & mamaField, string strVal, RsslFieldId fid )
{
	...
	case RSSL_DT_ENUM_AS_MAMA_FIELD_TYPE_STRING:
		...
		{
			return mamaMsg_addString(msg,  mamaField.mama_field_name.c_str(), mamaField.mama_fid, strVal.c_str() );
		}
