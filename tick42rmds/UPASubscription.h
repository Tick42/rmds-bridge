...
public:
    ...
    virtual bool Open(UPAConsumer_ptr_t consumer);
    virtual bool Close();
    // use this one for a standalone snapshot
    virtual bool Snapshot( UPAConsumer_ptr_t consumer, RMDSBridgeSnapshot_ptr_t snapRequest );

    // and this one to insert a refresh into an existing subscription stream
    virtual bool Refresh( UPAConsumer_ptr_t consumer, RMDSBridgeSubscription_ptr_t subRequest );
    ...
    // manage listeners
    void AddListener(RMDSBridgeSubscription_ptr_t listener);
    void RemoveListener(RMDSBridgeSubscription_ptr_t listener);
    ...
    void RequestImage(RMDSBridgeSubscription_ptr_t sub);

    void QueueSubscriptionDestroy(RMDSBridgeSubscription_ptr_t & sub);
...
protected:
    ...
    void NotifyListenersRefreshMessage(mamaMsg msg, RMDSBridgeSubscription_ptr_t sub, bool bookMessage = false);
...
private:
   ...
   void QueueOpenRequest();
    ...
    {
    public:
        UPASubscriptionClosure(UPASubscription_ptr_t s)
            :sub_(s)
        {}

        UPASubscription_ptr_t GetPtr() const
        {
            return sub_;
        }
    private:
    ...
    {
    public:
        RMDSBridgeSubscriptionClosure(RMDSBridgeSubscription_ptr_t s)
            :sub_(s)
        {}

        ~RMDSBridgeSubscriptionClosure();

        RMDSBridgeSubscription_ptr_t GetPtr() const
        {
            return sub_;
        }
    private:
